...
**The refresh mechanism (identified by on-device trace, implemented as Path A — pure JS):** the portal authenticates via **Microsoft Entra ID / MSAL.js**. `refreshAppCookie()` in `doch1.js` revives a dead `AppCookie` with two headless HTTP calls: (1) redeem the MSAL **refresh token** at `https://login.microsoftonline.com/{tenant}/oauth2/v2.0/token` (`grant_type=refresh_token`, `scope=User.Read openid profile offline_access`, `Origin: https://one.prat.idf.il`) → fresh `id_token`; (2) `GET /api/account/login` with `Authorization: <id_token>` (the **raw** id_token, no `Bearer ` prefix) → backend Set-Cookies a fresh `AppCookie`. The refresh token is captured from MSAL's `localStorage` during real WebView logins (`MSAL_RT_CAPTURE_JS` + `handleLoginWebViewMessage` in `useLoginDetection.js`), stored in AsyncStorage (`doch1_msal_rt`), and rotated on each use. `refreshAppCookie()` returns `{ ok, attempted }`; `attempted` is now always `true` when a refresh is tried, so a genuine failure (dead refresh token) drives the once-per-death "re-login" notification. Validate on-device with the **"Test headless refresh"** button in `TestConnectionScreen`.

**Longevity nuance:** Azure caps SPA refresh tokens at ~24h per hop but rotates them on each use, so the ~6h worker keeps the chain alive on a normally-used phone. A phone asleep >24h lets the token expire → one app-open reseeds it. For a guaranteed month through long sleeps, the fallbacks are the Azure SSO cookie (`ESTSAUTHPERSISTENT`, already in the native jar) via a silent `/authorize?prompt=none` flow, or **Path B** (native background `WebView`; `SYSTEM_ALERT_WINDOW` already in the manifest, needs prebuild).
