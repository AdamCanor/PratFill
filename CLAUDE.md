...
- The background task (`src/tasks/autoSubmitTask.js`) is refresh-then-submit: `refreshAppCookie()` → on success `runAutoSubmit()`; on failure notify **only while coverage is actually at risk** (`shouldNotifyAuthFailure()`: nothing stored yet, or the filled window ends within 2 days), once per death. A failed run is otherwise not worth waking the user — AppCookie dying between runs is the normal state and the next fire is ~6h away. The wording follows `refreshAppCookie()`'s `transient` flag: "פג תוקף ההתחברות" only for a real auth verdict, "לא הצלחנו להתחבר לשרת" when the request never completed. It fires every ≥360 **minutes** (`expo-background-task`'s `minimumInterval` is in minutes — `15 * 60` once meant ~15 hours here, a unit bug; ~6h so at least one fire lands per day under Doze). Registration re-runs when the interval constant changes.
- `runAutoSubmit()` owns the **state** notification, gated by `autoSubmit.successNotify` (`'daily'` | `'weekly'`, set in `SettingsAutoSubmitScreen`); **every** successful run sends it, including the "nothing to fill, you're covered" case (`כל 7 הימים הקרובים מדווחים`) — silence is indistinguishable from the task never running, and the point of a filler the user never opens is knowing where they stand. Days the preset deliberately leaves blank are counted honestly (`5 מתוך 7 הימים הקרובים מדווחים`). Coverage is persisted separately from the last-run record (`doch1_auto_submit_coverage`). A hard cap (`canSendNotificationToday()`/`markNotificationSent()`, shared across both notification types) additionally limits the app to **at most one notification per rolling 24h, period** — a backstop against edge cases neither type's own throttle covers alone (e.g. a partial-fill retry succeeding later the same day, or a session dying mid-day right after an earlier success).
