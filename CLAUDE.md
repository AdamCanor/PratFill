...
**Session recovery model (how the app copes):**
- On launch with a dead session, `RootNavigator` mounts the hidden `SessionRefreshWebView` behind the splash — a 1x1 invisible WebView that replays the silent login. The visible `LoginScreen` is only the fallback (genuinely-expired ~2-week login). Both share detection logic via `src/hooks/useLoginDetection.js`.
- After successful auth (and on foreground transitions, via an `AppState` listener in `App.js`), `runAutoSubmitIfStale()` runs a debounced catch-up auto-submit — so opening the app once in a while keeps the week filled even though background fires >~5h after the last app open always fail with `AuthError`.
- The background task (`src/tasks/autoSubmitTask.js`) fires every ≥120 **minutes** (`expo-background-task`'s `minimumInterval` is in minutes — `15 * 60` once meant ~15 hours here, a unit bug). On `AuthError` it notifies only when the filled window is within ~2 days of running out, and at most once per cookie-death (flag cleared on the next successful run). Coverage is persisted separately from the last-run record (`doch1_auto_submit_coverage`).
